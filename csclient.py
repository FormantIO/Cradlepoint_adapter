...
This file contains confidential information of CradlePoint, Inc.
and your use of this file is subject to the CradlePoint
Software License Agreement distributed with this file.
Unauthorized reproduction or distribution of this file is subject to civil
...
class CSClient(object):
    """
    The CSClient class is the NCOS SDK mechanism for communication
    between apps and the router tree/config store.
    Instances of this class communicate with the router using either
    an explicit socket or with http method calls.

    Apps running locally on the router use a socket on the router to
    send commands from the app to the router tree
    and to receive data (JSON) from the router tree.

    Apps running remotely use the requests library to send HTTP
    method calls to the router and to receive data from
    the router tree. This allows one to use an IDE to run and
    debug the application on a the computer. Although,
    there are limitations with respect to the device hardware
    access (i.e. serial, USB, etc.).
    """

    END_OF_HEADER = b"\r\n\r\n"
    STATUS_HEADER_RE = re.compile(b"status: \w*")  # noqa: W605
    CONTENT_LENGTH_HEADER_RE = re.compile(b"content-length: \w*")  # noqa: W605
    ...
    def get(self, base, query="", tree=0):
        ...
        The behavior of this method is contextual:
            ...
            queries the router tree to retrieve the
              specified data.
            - If the app running remotely from a computer it calls the HTTP GET method
    ...
    def put(self, base, value="", query="", tree=0):
        """
        Constructs and sends a put request to update or add specified data to the
        device router tree.

        The behavior of this method is contextual:
            - If the app is installed on(and executed from) a device, it directly
            updates or adds the specified data to
              the router tree.
            - If the app running remotely from a computer it calls the HTTP PUT
            method to update or add the specified
              data.
    ...
    def post(self, base, value="", query=""):
        """
        Constructs and sends a post request to update or add specified data to the
        device router tree.
        ...
        The behavior of this method is contextual:
            - If the app is installed on(and executed from) a device, it directly
              updates or adds the specified data to
              the router tree.
            - If the app running remotely from a computer it calls the HTTP
              POST method to update or add the specified
              data.
    ...
    def delete(self, base, query="", value=""):
        """
        Constructs and sends a delete request to delete specified data to the device
        router tree.

        The behavior of this method is contextual:
            - If the app is installed on(and executed from) a device, it directly
              deletes the specified data to
              the router tree.
            - If the app running remotely from a computer it calls the HTTP DELETE
              method to update or add the specified
              data.
    ...
    def alert(self, value=""):
        """
        Constructs and sends a custom alert to NCM for the device. Apps calling
        this method must be running
        on the target device to send the alert. If invoked while running on a computer,
        then only a log is output.
    ...
    def _get_device_access_info():
        ...
        if sys.platform != "linux2":
            ...
            # settings_file = os.getenv("CRADLE_POINT_SETTINGS_FILE")
            settings_file = os.getcwd()
            settings_file += "/sdk_settings.ini"
    ...
    def _safe_receive(self, sock):
        ...
        eoh = -1
        while eoh < 0:
            # In the event that the config store times out in returning data, lib
            # returns
...
class EventingCSClient(CSClient):
    ...
    def start(self):
        ...
        # backlog is optional. already set on value found in /proc
        self.event_sock.listen()
            ...
            try:
                ...
                for f, ev in events:
                    if ev & (select.POLLERR | select.POLLHUP):
                        ...
                        # TODO: restart w/ cached registrations.
                        # Will no longer be an error case
                        self.stop()
                    ...
                    if ev & select.POLLIN:
                        ...
                        try:
                            ...
                            try:
                                # PUTting just a string to config store results in a
                                # json encoded string returned.
                                # e.g. set /config/system/logging/level "debug",
                                # result['data']['cfg'] is '"debug"'
                            ...
                            try:
                                ...
                                if traceback:
                                    traceback.print_exc()
                                self.log(
                                    "Exception during callback for %s"
                                    % str(self.registry[eid])
                                )
                            if (
                                ...
                                # No dispatch.
                                # Config store receiver will put to config store.
                                conn.sendall(cb_return.encode())
...
def clean_up_reg(signal, frame):
    """
    When 'cppython remote_port_forward.py' gets a SIGTERM, config_store_receiver.py
    doesn't
    clean up registrations. Even if it did, the comm module can't rely on an
    external service
    to clean up.
    """
